...
def checker(wavfile,nfft,nceps):
    # 音声をロード
...
if __name__ == "__main__":
    nfft=2048
    nceps=12
    noiseFolder = "./learning_sample"
    noiseFolder = noiseFolder+"/*"
    noiseList = glob.glob(noiseFolder)
    
    for noise in noiseList:
        tmp=checker(noise,nfft,nceps)
        try:
            tmp=checker(noise,nfft,nceps)
            print("{0} OK.".format(noise))
        except:
            print("faild. {0} file do not used.".format(noise))
